...
def get_dice_torch(mask_true, mask_pred):
    """Mean dice score.
    ...
    mask_true : torch.tensor, 4d
         Tensor of ground truth masks or one mask that consists of 2 unique values:
         0 - denotes background, 1 - denotes object.
    mask_pred : torch.tensor, 4d
         Tensor of predicted masks or one mask that consists of 2 unique values:
         0 - denotes background, 1 - denotes object.
    ...
    """
    mask_pred = mask_pred.round()
    mts = []
    mps = []
    for mt, mp in zip(mask_true, mask_pred):
        mt = np.array(transforms.ToPILImage()(mt))
        mp = np.array(transforms.ToPILImage()(mp))
        mts.append(mt)
        mps.append(mp)
    mts = np.array(mts)
    mps = np.array(mps)
    return get_dice(mts, mps)
